...
struct Vec256<c10::qint8> {
 ...
 public:
  ...
  void __inline_attrs store(void* ptr, int count = size()) const {
    if (count != size()) {
      ...
      __vchari* vtmp = reinterpret_cast<__vchari*>(tmp_values);
      vtmp[0] = _vec0;
      vtmp[1] = _vec1;
      std::memcpy(ptr, tmp_values, count * sizeof(value_type));
    } else {
      vec_vsx_st(_vec0, offset0, reinterpret_cast<__vchari*>(ptr));
      vec_vsx_st(_vec1, offset16, reinterpret_cast<__vchari*>(ptr));
    }
  }

  static __inline_attrs Vec256<c10::qint8> loadu(const void* ptr) {
 ...
 public:
  float_vec_return_type __inline_attrs dequantize(
    ...
    return {
        Vec256<float>{vec_madd(scale_vec0, vecf0_0, scale_zp_premul0),
                      vec_madd(scale_vec1, vecf1_0, scale_zp_premul1)},
        Vec256<float>{vec_madd(scale_vec0, vecf0_1, scale_zp_premul0),
                      vec_madd(scale_vec1, vecf1_1, scale_zp_premul1)},
        Vec256<float>{vec_madd(scale_vec0, vecf0_2, scale_zp_premul0),
                      vec_madd(scale_vec1, vecf1_2, scale_zp_premul1)},
        Vec256<float>{vec_madd(scale_vec0, vecf0_3, scale_zp_premul0),
...
struct Vec256<c10::quint8> {
 ...
 public:
  ...
  void __inline_attrs store(void* ptr, int count = size()) const {
    if (count != size()) {
      value_type tmp_values[size()];
      __vchar* vtmp = reinterpret_cast<__vchar*>(tmp_values);
      vtmp[0] = _vec0;
      vtmp[1] = _vec1;
      std::memcpy(ptr, tmp_values, count * sizeof(value_type));
    } else {
      vec_vsx_st(_vec0, offset0, reinterpret_cast<__vchar*>(ptr));
      vec_vsx_st(_vec1, offset16, reinterpret_cast<__vchar*>(ptr));
    }
  }

  static __inline_attrs Vec256<c10::quint8> loadu(const void* ptr) {
    return Vec256<c10::quint8>{
        vec_vsx_ld(offset0, reinterpret_cast<const __vchar*>(ptr)),
        vec_vsx_ld(offset16, reinterpret_cast<const __vchar*>(ptr))};
  }
...
struct Vec256<c10::qint32> {
 ...
 public:
  ...
  loadu(const void* ptr, int64_t count = size()) {
    ...
    return Vec256<c10::qint32>{vtmp[0], vtmp[1]};
  }
  void __inline_attrs store(void* ptr, int count = size()) const {
    if (count == size()) {
  ...
  static Vec256<c10::qint32> quantize(
    ...
    constexpr int32_t min_val = std::numeric_limits<value_type>::min();
    constexpr int32_t max_val = std::numeric_limits<value_type>::max();
    const __vi vmin = vec_splats(min_val);
    const __vi vmax = vec_splats(max_val); 
    __vf inverse_scale_v = vec_splats(inverse_scale);
    __vf vec_zero_point = vec_splats((float)zero_point);
    Vec256<float> vf0 = rhs[0]; 
    __vf vecf0 = vf0.vec0();
