        ...
        namespace {
            ...
            public:
                ...
                Vec256<float> __inline_attrs exp() const {
                    ...
                    __vf x0, x1, fx0, fx1, z0, z1, y0, y1, tmp0, tmp1;
                    __vi imm0, imm1, imm00,imm01,imm10,imm11;
                    __vib mask0, mask1;

                    const __vf one = vec_splats(1.f);
                    const __vf exp_hi = vec_splats(104.f);
                    const __vf exp_lo = vec_splats(-104.f);
                    const __vf LOG2EF = vec_splats(1.44269504088896341f);
                    ...
                    const __vf inf = (__vf)vec_splats(0x7f800000u);
                    const __vi v0x7f = vec_splats(0x7f);
                    __vf zerov = { 0,0,0,0 };
                    x0 = _vec0;// vec_min(_vec0, exp_hi);
                    x1 = _vec1;// vec_min(_vec1, exp_hi);
                    ...
                    /* Express e**x = e**g 2**n
                      ...
                      *   = e**( g + n loge(2) )
                      */ 
                    fx0 = vec_round(vec_mul(x0, LOG2EF));
                    fx1 = vec_round(vec_mul(x1, LOG2EF));  
                     
                    x0 = vec_madd(fx0 , negln2f_hi , x0);
                    x1 = vec_madd(fx1 , negln2f_hi , x1);
            
                    x0 = vec_madd(fx0, negln2f_lo, x0);
                    x1 = vec_madd(fx1, negln2f_lo , x1);
                  
                    z0 = vec_mul(x0, x0);
                    z1 = vec_mul(x1, x1);
                    
                    y0 = vec_madd(exp_p0, x0, exp_p1);
                    y1 = vec_madd(exp_p0, x1, exp_p1);
                     
                    y0 = vec_madd(y0, x0, exp_p2);
                    y1 = vec_madd(y1, x1, exp_p2);
                     
                    y0 = vec_madd(y0, x0, exp_p3);
                    y1 = vec_madd(y1, x1, exp_p3);
                     
                    y0 = vec_madd(y0, x0, exp_p4);
                    y1 = vec_madd(y1, x1, exp_p4);
                     
                    y0 = vec_madd(y0, x0, exp_p5);
                    y1 = vec_madd(y1, x1, exp_p5);
                    ...
                    /* vm_pow2n 2^n */ 
                     imm0 = vec_signed(fx0);
                     imm1 = vec_signed(fx1);  
                    //this pow2n was from sleef code
                    imm00 = imm0 >> 1; //>>1 
                    imm01 = imm1 >> 1;  
                    imm10 = imm0 - imm00;
                    imm11 = imm1 - imm01;
                    imm00 =  (imm00 + v0x7f ) << v23 ;
                    imm01 =  (imm01 + v0x7f)  << v23 ;
                    imm10 = (imm10 + v0x7f)   << v23;
                    imm11 = (imm11 + v0x7f)   << v23;
                    // treat imm0 as float vector without conversion

                    y0 = (y0 * (__vf)imm00) * (__vf)imm10;
                    y1 = (y1 * (__vf)imm01) * (__vf)imm11; 
                    //boundary check

                    mask0 = vec_cmple(exp_hi, _vec0);
                    mask1 = vec_cmple(exp_hi, _vec1);
                    tmp0 = vec_sel(y0, inf, mask0);
                    tmp1 = vec_sel(y1, inf, mask1);
                    mask0 = vec_cmplt(_vec0, exp_lo);
                    mask1 = vec_cmplt(_vec1, exp_lo);
                    y0 = vec_sel(tmp0, zerov, mask0);
                    y1 = vec_sel(tmp1, zerov, mask1);
                ...
                Vec256<float> __inline_attrs log() const {
                    __vi imm0, imm1;
                    __vf x0, x1, e0, e1, y0, y1, z0, z1, tmp0, tmp1;
                    __vib mask0, mask1;
                    const __vf one = vec_splats(1.f);
                    const __vui min_norm_pos = vec_splats(0x00800000u);
                    const __vui inv_mant_mask = vec_splats((unsigned int)~0xff800000);
                    const __vf min_inf = (__vf)vec_splats(0xff800000u);
                    const __vf nan = (__vf)vec_splats(0x7fffffff);
                    const __vi v0x7f = vec_splats(0x7f);
                    const __vui v23 = vec_splats(23u);
                    ...
                    __vib invalid_mask0 = (__vib)vec_cmplt(_vec0, zero);
                    __vib invalid_mask1 = (__vib)vec_cmplt(_vec1, zero);
                    ...
                    e0 = vec_add(vec_float(imm0), one);
                    e1 = vec_add(vec_float(imm1), one);
                    /* part2:
                                              if( x < SQRTHF ) {
                                                    e -= 1;
                                                    x = x + x - 1.0;
                    ...
                    y0 = vec_madd(x0, log_p0, log_p1);
                    y1 = vec_madd(x1, log_p0, log_p1);
                    y0 = vec_madd(y0, x0, log_p2);
                    y1 = vec_madd(y1, x1, log_p2);
                    y0 = vec_madd(y0, x0, log_p3);
                    y1 = vec_madd(y1, x1, log_p3);
                    y0 = vec_madd(y0, x0, log_p4);
                    y1 = vec_madd(y1, x1, log_p4);
                    y0 = vec_madd(y0, x0, log_p5);
                    y1 = vec_madd(y1, x1, log_p5);
                    y0 = vec_madd(y0, x0, log_p6);
                    y1 = vec_madd(y1, x1, log_p6);
                    y0 = vec_madd(y0, x0, log_p7);
                    y1 = vec_madd(y1, x1, log_p7);
                    y0 = vec_madd(y0, x0, log_p8);
                    y1 = vec_madd(y1, x1, log_p8);
                    y0 = vec_mul(y0, x0);
                    y1 = vec_mul(y1, x1);
                    ...
                    y0 = vec_madd(e0, log_q1, y0);
                    y1 = vec_madd(e1, log_q1, y1);

                    y0 = y0 - z0 * _0p5;
                    y1 = y1 - z1 * _0p5;
                    x0 = vec_add(x0, y0);
                    x1 = vec_add(x1, y1);
                    x0 = vec_madd(e0, log_q2, x0);
                    x1 = vec_madd(e1, log_q2, x1);

                    x0 = vec_sel(x0, nan, invalid_mask0); // negative arg will be NAN
                    x1 = vec_sel(x1, nan, invalid_mask1);

                    //zero is -inf 
                    mask0 = vec_cmpeq(_vec0, zero);
                    mask1 = vec_cmpeq(_vec1, zero);
                    x0 = vec_sel(x0, min_inf, mask0);
                    x1 = vec_sel(x1, min_inf, mask1);
                ...
                Vec256<float> __inline_attrs cos() const {
                    ...
                    const __vib sign_mask = (__vib)vec_splats(0x80000000);
                ...
                Vec256<float> __inline_attrs cosh() const {
                    ...
                    Vec256<float> ret = abs().exp();
                    recp_vec0 = vec_div(one, ret._vec0);
                    recp_vec1 = vec_div(one, ret._vec1);
                    v0 = vec_add(ret._vec0, recp_vec0);
                    v1 = vec_add(ret._vec1, recp_vec1);
                    v0 = vec_mul(half, v0);
                    v1 = vec_mul(half, v1);
                ...
                Vec256<float> __inline_attrs sinh() const {
                    ...
                    __vf recp_vec0, recp_vec1, v0, v1, z0, z1, y0, y1;
                    __vib sign_bit0, sign_bit1;
                    __vf vabs0 = vec_abs(_vec0);
                    __vf vabs1 = vec_abs(_vec1); 
                    //get exponent
                    auto ret = Vec256<float>(vabs0, vabs1).exp(); 

                    recp_vec0 = vec_div(half, ret._vec0);
                    recp_vec1 = vec_div(half, ret._vec1);

                    v0 = half * ret._vec0 - recp_vec0;
                    v1 = half *ret._vec1 - recp_vec1;

                    /* extract the sign bit (upper one) */
                    sign_bit0 = (__vib)vec_and(_vec0, sign_mask);
                    sign_bit1 = (__vib)vec_and(_vec1, sign_mask);
                    v0 = vec_or(v0, sign_bit0);
                    v1 = vec_or(v1, sign_bit1); 
                    ...
                    y0 = y0 * z0 + p2;
                    y1 = y1 * z1 + p2;
                    y0 = y0 * z0 * vabs0 + vabs0;
                    y1 = y1 * z1 * vabs1 + vabs1;


                    //check and select
                    __vib sel_mask0 = (__vib)vec_cmpgt(vabs0, one);
                    __vib sel_mask1 = (__vib)vec_cmpgt(vabs1, one);
                   // __vib max_mask0 = (__vib)vec_cmpgt(vabs0, max_logf);
                   // __vib max_mask1 = (__vib)vec_cmpgt(vabs1, max_logf);

                    //__vf max_ret0 = vec_xor(sign_bit0, max_numf);
                    //__vf max_ret1 = vec_xor(sign_bit1, max_numf); //-1
                    v0 = vec_sel(y0, v0, sel_mask0);
                    v1 = vec_sel(y1, v1, sel_mask1);
                   // v0 = vec_sel(v0, max_ret0, max_mask0);
                   // v1 = vec_sel(v1, max_ret1, max_mask1);
                ...
                Vec256<float> __inline_attrs tanh() const {
                    ...
                    __vf vabs0, vabs1, v2x0, v2x1, vv0, vv1, z0, z1, y0, y1, tmp0, tmp1;
                    __vf max_ret0, max_ret1, v0, v1;
                    __vib sign_bit0, sign_bit1, sel_mask0, sel_mask1, max_mask0, max_mask1;

                    vabs0 = vec_abs(_vec0);
                    vabs1 = vec_abs(_vec1);
                    v2x0 = vec_add(vabs0, vabs0);
                    v2x1 = vec_add(vabs1, vabs1);
                    //get exponent
                    auto exp2x = Vec256<float>(v2x0, v2x1).exp();
                    vv0 = vec_add(exp2x._vec0, one);
                    vv1 = vec_add(exp2x._vec1, one);
                    vv0 = vec_sub(one, vec_div(two, vv0));
                    vv1 = vec_sub(one, vec_div(two, vv1));
                    ...
                    z0 = vec_mul(vabs0, vabs0);
                    z1 = vec_mul(vabs1, vabs1);
                    y0 = vec_madd(tanh_p0, z0, tanh_p1);
                    y1 = vec_madd(tanh_p0, z1, tanh_p1);
                    tmp0 = vec_madd(y0, z0, tanh_p2);
                    tmp1 = vec_madd(y1, z1, tanh_p2);
                    y0 = vec_madd(tmp0, z0, tanh_p3);
                    y1 = vec_madd(tmp1, z1, tanh_p3);
                    tmp0 = vec_madd(y0, z0, tanh_p4);
                    tmp1 = vec_madd(y1, z1, tanh_p4);
                    y0 = vec_mul(tmp0, z0);
                    y1 = vec_mul(tmp1, z1);
                    tmp0 = vec_madd(y0, _vec0, _vec0);
                    tmp1 = vec_madd(y1, _vec1, _vec1);

                    //add sign
                    vv0 = vec_or(vv0, sign_bit0);
                    vv1 = vec_or(vv1, sign_bit1);

                    //check and select
                    sel_mask0 = (__vib)vec_cmpge(vabs0, tanh_0p625);
                    sel_mask1 = (__vib)vec_cmpge(vabs1, tanh_0p625);
                    max_mask0 = (__vib)vec_cmpgt(vabs0, tanh_half_max);
                    max_mask1 = (__vib)vec_cmpgt(vabs1, tanh_half_max);

                    max_ret0 = vec_xor(sign_bit0, one);
                    max_ret1 = vec_xor(sign_bit1, one); //-1
                    v0 = vec_sel(tmp0, vv0, sel_mask0);
                    v1 = vec_sel(tmp1, vv1, sel_mask1);
                    vv0 = vec_sel(v0, max_ret0, max_mask0);
                    vv1 = vec_sel(v1, max_ret1, max_mask1);

                    return Vec256<float>{vv0, vv1};
                }
                ...
                Vec256<float> __inline_attrs reciprocal() const {
                    const __vf one = vec_splats(1.f);
                    return Vec256<float>{
                        vec_div(one, _vec0), // vec_re(_vec0) - is estimated calc so we dont use it
                        vec_div(one, _vec1)};
                }
                ...
                Vec256<float> __inline_attrs operator==(const Vec256<float>& other) const {
                    return Vec256<float>{
                        (__vib)vec_cmpeq(_vec0, other._vec0),
                        (__vib)vec_cmpeq(_vec1, other._vec1)};
                }

                Vec256<float> __inline_attrs operator!=(const Vec256<float>& other) const {
                    return Vec256<float>{
                        (__vib)vec_cmpne(_vec0, other._vec0),
                        (__vib)vec_cmpne(_vec1, other._vec1)};
                }

                Vec256<float> __inline_attrs operator<(const Vec256<float>& other) const {
                    return Vec256<float>{
                        (__vib)vec_cmplt(_vec0, other._vec0),
                        (__vib)vec_cmplt(_vec1, other._vec1)};
                }

                Vec256<float> __inline_attrs operator<=(const Vec256<float>& other) const {
                    return Vec256<float>{
                        (__vib)vec_cmple(_vec0, other._vec0),
                        (__vib)vec_cmple(_vec1, other._vec1)};
                }

                Vec256<float> __inline_attrs operator>(const Vec256<float>& other) const {
                    return Vec256<float>{
                        (__vib)vec_cmpgt(_vec0, other._vec0),
                        (__vib)vec_cmpgt(_vec1, other._vec1)};
                }

                Vec256<float> __inline_attrs operator>=(const Vec256<float>& other) const {
                    return Vec256<float>{
                        (__vib)vec_cmpge(_vec0, other._vec0),
